...
def register_user_device(username: str, password: str, email: str = None) -> Status:
    ...
    if isinstance(ret, str):
        logger_general.info(f"Failed to register: {ret}")
        return Status.fail(ret)
    else:
        logger_general.info(f"Successfully registered")
...
def login_manual(username: str, password: str, allow_auto_login=True) -> Status:
    ...
    if isinstance(ret, str):
        logger_general.info(f"Failed to login manually: {ret}")
        return Status.fail(ret)
    else:
        print("Successfully logged in")
        if allow_auto_login:
            _save_received_token(ret)
        logger_general.info(f"Successfully logged in manually")
...
def logout() -> Status:
    ...
    logger_general.info(f"Successfully logged out")
...
def login_auto() -> Status:
    ...
    if token is not None:
        ...
        if not success:
            logger_general.info(f"Failed to login automatically: Wrong token")
            return Status.fail("Failed to automatically log in.")
        else:
            logger_general.info(f"Successfully logged in automatically")
            return Status.success("Successfully auto logged in")

    else:
        logger_general.info(f"Failed to login automatically: Token not exists")
        return Status.fail("Failed to automatically log in.")
